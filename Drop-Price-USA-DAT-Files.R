...
# identify the path that contains the following files:
  # base-rbm-52B-USA.csv
  # wgnip-market-USA.csv
  # dosage-current.csv
  # production-cost.csv
  # market-mapping-USA.csv
...
folder = "DataIn-Baseline"

# choose a path to place "folder"
output.files = "/shared/kgcoe-research/gates/Budget-Uncertainty/R-Files/Fall-2017/Drop-Price-USA-52B-14M"
...
# assign a value of TRUE or FALSE to baseline
# a value of TRUE means reservation prices will not be dropped, and just baseline dat files for different values of MARR will be created
baseline = TRUE
...
baserbm = data.table(read.csv("base-rbm-52B-USA.csv"))
wgnip = data.table(read.csv("wgnip-market-USA.csv"))
dosage = data.table(read.csv("dosage-current.csv"))
cost = data.table(read.csv("production-cost.csv"))
mapping = data.table(read.csv("market-mapping-USA.csv"))

# extract the number of bundles from rbm
bundles = max(baserbm$Bundle)

# extract the different market segments from dosage
markets = sort(unique(baserbm$Markets))

# extract the different MARR values from cost
marr = sort(unique(cost$MARR))

# add two markets to each market segmentation in dosage
dosage = foreach(i = sort(unique(dosage$Markets))) %do%
{
  # extract market segmentation i
  output = data.table(dosage[Markets == i])
  
  # add 2 markets to Markets and MarketID in output
  output[, Markets := Markets + 2]
  output[, MarketID := MarketID + 2]
  
  # extract market segmentation i once more
  output2 = data.table(dosage[Markets == i])
  
  # add 2 markets to Markets in output2
  output2[, Markets := Markets + 2]
  
  # extract just the first two markets from output2
  output2 = output2[MarketID %in% 1:2]
  
  # append output to output2
  output = rbind(output2, output)
  
  return(output)
}

# combine the list of data tables into 1 table
dosage = rbindlist(dosage)

# order dosage by Markets, Antigen, and MarketID
dosage = dosage[order(Markets, Antigen, MarketID)]
...
# if baseline is TRUE then build a baseline experiment otherwise build a price drop experiment
if(baseline == TRUE)
{
  ...
  {
    # for Market Segmentation i, create all combination of:
      # MARR
    output = data.table(expand.grid(MARR = marr,
                                    Markets = i))
  ...
  # get the baseline reservation prices for each bundle
  doe.dat = foreach(i = 1:nrow(doe)) %do%
  {
...
} else
{
  # lets build our experiment
    # the market segmentations will vary according to "markets"
    # the USA-Public market will experience a random price drop
  ...
  doe = foreach(i = markets) %do%
  {
    ...
    # this will ensure that only the USA-Public market will face a price drop
    market.output = foreach(j = mapping[Markets == i & USA == "Public", MarketID]) %do%
    {
...
doe.dat[, fileID := Scenario]

# lets create an object DT that will hold the ampl dat file information
DT = data.table(doe.dat[, .(fileID, Markets, Bundle, MarketID, MARR, baseRbm)])
...
foreach(i = 1:max(DT$fileID)) %fun%
{
  ...
  write.table(ampl(DT[fileID == i, .(Bundle, MarketID, baseRbm)], name = "R"), 
